...
def roll_weights(weights: Tuple[np.ndarray, ...]) -> np.ndarray:
    """
    Rolls the weights into a single vector.

    :param weights: The weights as a tuple of numpy arrays.
    :return: The weights as a single vector.
    """
...
def read_weights(path: Path, column_names: Tuple[str, ...]) -> Tuple[np.ndarray, ...]:
    ...
    result[1] = np.roll(result[1], 1, axis=0)
    return tuple(result)
